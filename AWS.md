...
Enabling Multi-Factor Authentication (MFA) for AWS IAM users adds an extra layer of security. Here’s a step-by-step guide:
...
Click on the IAM user for whom you want to enable MFA.
...
Hardware MFA Device: Use a physical security key or device.



6. Configure the Device

Follow the instructions to scan the QR code (if using a virtual MFA device).

Enter the two consecutive MFA codes generated by the app to complete the setup.


7. Verify and Save

Once verified, click Assign MFA to enable MFA for the user.
...
Would you like more details on configuring a specific device?
